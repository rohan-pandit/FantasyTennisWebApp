...
import model.Player.Validation;
...
public class DbMods {
    ...
    public String getPlayer(int userId) {
        ...
        String sql = "SELECT name FROM atp_player WHERE fk_fantasy_user=?";
        String playerNames = "";
        ...
        try {
            ...
            try {
                ...
                while (results.next()) {
                    playerNames += results.getString("name") + ",";
    ...
    public String storeWins(int wins, int userId) {
        this.errorMsg = "";// empty error message means it worked.

        String sql = "UPDATE fantasy_user SET wins=?"
                + " WHERE idfantasy_user=?";
